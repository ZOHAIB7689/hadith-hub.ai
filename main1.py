...
# Function to read and update search count globally
def read_search_count() -> int:
    """Read the global search count from a file."""
    if not os.path.exists(SEARCH_COUNT_FILE):
        with open(SEARCH_COUNT_FILE, "w") as f:
            f.write("0")
    with open(SEARCH_COUNT_FILE, "r") as f:
        return int(f.read().strip())

def update_search_count():
    """Increment and update the global search count."""
    count = read_search_count() + 1
    with open(SEARCH_COUNT_FILE, "w") as f:
        f.write(str(count))
    return count
...
def initialize_session_state():
    ...
    if 'search_count' not in st.session_state:
        st.session_state.search_count = read_search_count()
...
# Display sidebar history and search count
def display_sidebar():
    """Display search history and search count in sidebar."""
    st.sidebar.title("📊 Search Statistics")
    
    # Display global search count
    st.sidebar.metric("Total Searches", st.session_state.search_count)
...
def main():
    ...
    if st.button("🔍 Search", type="primary"):
        with st.spinner("Searching Islamic sources..."):
            ...
            st.session_state.search_count = update_search_count()

            st.write(f"### 🔎 Search Result:")
